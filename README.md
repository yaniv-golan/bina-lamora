...
> ### 👉 מורה או יועצת? זה העמוד בשבילך: **[hameshabetz — עמוד הבית הידידותי](https://yaniv-golan.github.io/bina-lamora/)**
> העמוד הזה (GitHub) מיועד למפתחים. ההסבר הפשוט, עם תמונות והדגמה, נמצא שם.
...
>
> [![Install in Claude Desktop](https://img.shields.io/badge/Install_in_Claude_Desktop-D97757?style=for-the-badge&logo=claude&logoColor=white)](https://yaniv-golan.github.io/bina-lamora/static/install-claude-desktop.html)
>
> </div>
>
...
5. בשדה הכתובת מקלידים:
   ...
   ```
   yaniv-golan/bina-lamora
   ```

   </div>

   ובוחרים את **bina-lamora / yaniv-golan** שקופץ ברשימה.
...
<img src="docs/images/frame2-sync.png" width="760" alt="חלון Add marketplace — הקלדת yaniv-golan/bina-lamora ולחיצה על Sync">

<sub>③ מקלידים <code>yaniv-golan/bina-lamora</code>, ④ לוחצים **Sync**.</sub>
...
`yaniv-golan/bina-lamora` (בדיוק הכתובת מהשלבים למעלה — ChatGPT קורא אותה ישירות),
...
ב-`hameshabetz-skill`) מ[עמוד ההורדות](https://github.com/yaniv-golan/bina-lamora/releases/latest),
...
**bina-lamora** ("Claude for the Teacher", בינה־למורה) is a Claude plugin marketplace
...
5. Enter `yaniv-golan/bina-lamora` and pick it from the list.
...
```
/plugin marketplace add yaniv-golan/bina-lamora
/plugin install hameshabetz@bina-lamora
```

**Any agent (npx)** — Claude Code, Cursor, Copilot, Windsurf, and [40+ others](https://github.com/vercel-labs/skills):

```bash
npx skills add yaniv-golan/bina-lamora
```

**Cursor** — open **Cursor Settings** and paste `https://github.com/yaniv-golan/bina-lamora`
into the **Search or Paste Link** box.

**Codex CLI** — `$skill-installer https://github.com/yaniv-golan/bina-lamora`, or download the
`hameshabetz-skill-*.zip` asset from the [latest release](https://github.com/yaniv-golan/bina-lamora/releases/latest)
...
- **Codex CLI:** `codex plugin marketplace add yaniv-golan/bina-lamora`, then
  `codex plugin install hameshabetz@bina-lamora`.
- **ChatGPT desktop app (updatable):** **Plugins → Create ⌄ → Add marketplace**, enter
  `yaniv-golan/bina-lamora`, then **Install**. Update later via `codex plugin marketplace upgrade`,
...
- **Web (Skills upload):** download the `hameshabetz-skill-*.zip` asset from the
  [latest release](https://github.com/yaniv-golan/bina-lamora/releases/latest) and upload it via
